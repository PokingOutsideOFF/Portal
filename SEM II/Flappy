...
using namespace std;
// cout<<"±±±±±±±±±";
HANDLE console = GetStdHandle(STD_OUTPUT_HANDLE);//GETSTD TAKES INPUT
COORD CursorPosition;//LIKE A STRUCT COORD
int pipePos[3];
int gapPos[3];
int pipeFlag[3];
char bird[2][6] = { '/','-','-','o','\\',' ',
...
void setcursor(bool visible, DWORD size) //DWORD IS 32 BIT UNSIGNED INT
{
if(size == 0)
size = 20;
...
gapPos[ind] = 3 + rand()%14; //rand gives random numbers
}
...
{
setcursor(0,0);
srand( (unsigned)time(NULL)); //generates random seq of obstacles
// play();
do{
system("cls");
gotoxy(10,5); cout<<" -------------------------- ";
